...
public class PongBoard implements Screen {
	...
	public class MainInputProcessor implements InputProcessor {
		...
		public boolean touchDragged(int screenX, int screenY, int pointer) {
			setPaddleLocationForTouchDragged(camera.unproject(tmpV.set(screenX,
					screenY, 0)));
			return true;
		}
	...
	private void paddleMoveTween(final Paddle paddle, Vector3 pos) {
		...
		Tween.to(paddle, PaddleAccessor.POSITION_Y, .15f).ease(Sine.INOUT)
				.target(pos.y).setCallback(tweenCallback)
	...
	private void startScreenShake() {
		if (allowScreenShake) {
			...
			Timeline.createSequence()
					.push(Tween.set(camera, CameraAccessor.POSITION_XY).target(
							400, 240))
					.push(Tween.to(camera, CameraAccessor.POSITION_XY, 0.035f)
							.targetRelative(8, 0).ease(Quad.IN))
					.push(Tween.to(camera, CameraAccessor.POSITION_XY, 0.035f)
							.targetRelative(-8, 0).ease(Quad.IN))
					.push(Tween.to(camera, CameraAccessor.POSITION_XY, 0.0175f)
							.target(400, 240).ease(Quad.IN)).repeatYoyo(2, 0)
					.start(game.tweenManager);
		}
	}
	...
	private void batchDraw() {
		...
		scoreFont.draw(game.batch, String.valueOf(player1Score), 200,
				HEIGHT - 50);
		scoreFont.draw(game.batch, String.valueOf(player2Score), WIDTH - 200,
				HEIGHT - 50);
	...
	private void beginIntroTween() {
		...
		Tween.to(camera, CameraAccessor.POSITION_X, 2f).targetRelative(-800)
				.ease(Back.OUT).setCallback(callBack).start(game.tweenManager);
	}
	...
	private void beginOutroTween() {
		...
		Timeline.createSequence()
				...
				.push(Tween.to(camera, CameraAccessor.POSITION_X, 2f)
						.targetRelative(-800).ease(Back.IN))
